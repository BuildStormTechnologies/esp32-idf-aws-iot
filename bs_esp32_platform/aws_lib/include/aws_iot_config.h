...
#define MAX_ACKS_TO_COMEIN_AT_ANY_GIVEN_TIME 3 ///< At Any given time we will wait for this many responses. This will correlate to the rate at which the shadow actions are requested
...
#define MAX_JSON_TOKEN_EXPECTED 10 ///< These are the max tokens that is expected to be in the Shadow JSON document. Include the metadata that gets published
#define MAX_SHADOW_TOPIC_LENGTH_WITHOUT_THINGNAME 100 ///< All shadow actions have to be published or subscribed to a topic which is of the formablogt $aws/things/{thingName}/shadow/update/accepted. This refers to the size of the topic without the Thing Name
...
#define AWS_IOT_MQTT_MIN_RECONNECT_WAIT_INTERVAL 100 ///< Minimum time before the First reconnect attempt is made as part of the exponential back-off algorithm
#define AWS_IOT_MQTT_MAX_RECONNECT_WAIT_INTERVAL 1000 ///< Maximum time interval after which exponential back-off will stop attempting to reconnect.
